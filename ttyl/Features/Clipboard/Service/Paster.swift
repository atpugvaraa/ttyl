...
enum Paster {
    /// Stamped on ttyl's own synthetic keystrokes so the snippet keyword tap can skip them.
    ...
    static func postCommandV(toPid pid: pid_t? = nil) {
        ...
        down.setIntegerValueField(.eventSourceUserData, value: ttylEventTag)
        up.setIntegerValueField(.eventSourceUserData, value: ttylEventTag)
