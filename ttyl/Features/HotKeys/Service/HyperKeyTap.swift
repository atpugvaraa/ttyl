...
private enum CapsLockRemap {
    ...
    private static let queue = DispatchQueue(label: "aaravgupta.ttyl.capslock-remap", qos: .utility)
    ...
    private static func apply(_ mapping: String) {
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                NSLog("ttyl: hidutil remap exited %d", process.terminationStatus)
            }
        } catch {
            NSLog("ttyl: hidutil caps lock remap failed: %@", error.localizedDescription)
        }
    }
}
