...
enum ShellCommandRunner {
    ...
    private static let queue = DispatchQueue(
        label: "aaravgupta.ttyl.shell-command", qos: .userInitiated, attributes: .concurrent)
    ...
    nonisolated private static func execute(
        ...
        // Lets a shell config skip slow sections when ttyl is the caller.
        process.environment = ProcessInfo.processInfo.environment.merging(["TTYL": "1"]) { _, new in
            new
        }
    ...
    nonisolated private static func makeStderrCapture() -> StderrCapture? {
        let template = FileManager.default.temporaryDirectory
            .appendingPathComponent("ttyl-command-stderr.XXXXXX").path
