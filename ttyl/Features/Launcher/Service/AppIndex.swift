...
final class AppIndex {
    ...
    private static let systemActionEntries: [AppEntry] = SystemActionCatalog.all
        .map { command in
            AppEntry(
                ...
                url: URL(string: "ttyl://system-action/" + command.id.rawValue)!,
    ...
    private static let allWindowCommandEntries: [AppEntry] = WindowCommandCatalog.all
        .map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "ttyl://window-command/" + command.id.rawValue)!,
    ...
    func setCustomCommands(_ commands: [CustomCommand]) {
        let entries = commands.map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "ttyl://custom-command/" + command.id.uuidString)!,
        ...
        let entries =
            ...
            .map { quicklink in
                AppEntry(
                    ...
                    url: URL(string: "ttyl://quicklink/" + quicklink.id.uuidString)!,
