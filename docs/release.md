...
```sh
./Scripts/build-dmg.sh            # -> build/ttyl-<version>.dmg (version from project.yml)
./Scripts/build-dmg.sh 0.5.7      # -> build/ttyl-0.5.7.dmg
```

It builds a Release `ttyl.app` signed with `ttyl Self-Signed` and packs it with an
`/Applications` symlink. Official per-channel releases are built by CI, below.

## Signing & Gatekeeper

Both local builds and CI releases sign with the same stable `ttyl Self-Signed` identity, not an
Apple Developer ID — so macOS quarantines a directly-downloaded DMG. The Homebrew cask strips that
automatically; direct downloaders run `xattr -dr com.apple.quarantine "…/ttyl.app"` once. Full
...
- **channel** — `beta` or `stable`. Each builds a distinct app (`ttyl Beta.app` / `ttyl.app`)
  with its own bundle id, alongside the local `ttyl Dev.app`. Beta gets an auto-incrementing
...
`v<full-version>` with a versioned DMG asset (`ttyl-<full-version>.dmg`), marked prerelease for
beta. On success it also bumps the matching cask in the tap.

### Homebrew tap automation

The release job's final step rewrites the `version` + `sha256` of the channel's cask (`ttyl` or
`ttyl@beta`) in the [`homebrew-ttyl`](https://github.com/atpugvaraa/homebrew-ttyl) tap and
...
`https://atpugvaraa.github.io/ttyl/` on every push to `main` that touches `website/`. Enable it
