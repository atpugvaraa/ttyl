...
Create the `ttyl Self-Signed` code-signing identity once — builds sign with it, which is what keeps
...
```sh
open ttyl.xcodeproj    # then ⌘R
```

Or from the command line:

```sh
xcodebuild -project ttyl.xcodeproj -scheme ttyl -configuration Debug build
```
...
`ttyl.xcodeproj` is committed and generated from `project.yml` via XcodeGen — after changing
...
Debug builds are a separate channel: **`ttyl Dev.app`**, bundle id `aaravgupta.ttyl.dev`. Every
...
- The dev build asks for Accessibility on its own the first time, and starts with **no** hotkeys bound
  ...
  and the `ttyl Self-Signed` identity keep the TCC grant alive.
...
xcodebuild -project ttyl.xcodeproj -scheme ttyl -configuration Debug \
    -derivedDataPath build/DerivedData build 2>&1 | tee /tmp/ttyl-build.log
./Scripts/sync-lsp.sh /tmp/ttyl-build.log
```
...
**Build ttyl.app (debug)** task (⌘⇧B) and **F5** re-run the script on every build, so new and
...
```sh
./Scripts/format.sh            # format ttyl/ and Tests/ in place
...
```sh
node Scripts/gen-emoji.js            # -> ttyl/Features/Emoji/Model/EmojiData.generated.swift
node Scripts/gen-currencies.js       # -> ttyl/Features/Calculator/Model/CurrencyData.generated.swift
```
