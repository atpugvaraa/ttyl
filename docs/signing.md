...
ttyl is signed with a **stable self-signed identity** called `ttyl Self-Signed`. It's not an
...
## 1. Create the `ttyl Self-Signed` identity (once)
...
openssl pkcs12 -export -inkey /tmp/tc-key.pem -in /tmp/tc-cert.pem \
  -name "ttyl Self-Signed" -out /tmp/tc.p12 -passout pass:ttyl
...
```sh
security find-identity -p codesigning | grep "ttyl Self-Signed"
```
...
```sh
gh secret set SIGNING_P12_BASE64   --repo atpugvaraa/ttyl < /tmp/signing.p12.base64
gh secret set SIGNING_P12_PASSWORD --repo atpugvaraa/ttyl --body "$P12_PASSWORD"
rm -f /tmp/signing.p12.base64   # holds your private key — delete it
```

If you ever lose the secrets, just re-run this section — as long as the `ttyl Self-Signed`
